    ...
    public class BounceDetectorSystem : IInitializableSystem, IUpdatableSystem
    {
        ...
        public void OnUpdate(float deltaTime)
        {
            if (_contacts.Count > 0)
            {
                ...
                {
                    if (Physics.Raycast(_previousPosition, _transform.position, out RaycastHit hit, 1000, _layerToBounceReaction.value))
                    {
        ...
        private bool MatchWithBounceLayer(Collider collider)
            => ((1 << collider.gameObject.layer) & _layerToBounceReaction) != 0;
    }
}